...
This folder contains a sample use case of Olive to optimize and quantize [google/vit-base-patch16-224](https://huggingface.co/google/vit-base-patch16-224) model using existing tools.

## Quantization Workflows
- CPU: *PyTorch Model -> Onnx Model -> Onnx Optimized Model -> OnnxQuantization 8 bit RTN Quantized Onnx Model*

## Config file 
The workflow in Config file: [vit_RTN_ONNX_8bit.json](vit_RTN_ONNX_8bit.json) executes the above workflow.
...
pip install -e .
cd examples/vit-base-patch16-224/nxp/
pip install -r requirements.txt
```
**NOTE:**

- For access to the [google/vit-base-patch16-224](https://huggingface.co/google/vit-base-patch16-224) model you'll need to log-in to Hugging Face with a [user access token](https://huggingface.co/docs/hub/security-tokens) so that Olive can download it.
...
```bash
olive run --config vit_RTN_ONNX_8bit.json
```
...
olive_run("vit_RTN_ONNX_8bit.json")
```
